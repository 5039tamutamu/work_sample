...
int main(int argc, char ** argv) {
	FILE *fp;
	char fileName[1000000000]= "./input_1W192241_20201203.txt";
	fpos_t size;
	struct tnode *root;
	char c;
	int i;
...
void parse(struct tnode *p) {
	...
	if (p->type == Prog) {
		...
		parse(p->left);//左の子に丸投げ
		if (tp != NULL) {//最後に余計な文字があった場合
			err("parse Prog: extra token at the end of input: ");//エラー表示
		}
		if (tp == NULL) {//最後に何もない場合
			printf("completed.\n");//構文解析終了(完了メッセージを表示)
		}
		...
		if (match("{")) {
			...
			parse(p->left);//Rep_com部分の構文解析の実行(左の子に丸投げ)

		} else if (match("walk") || match("move") || match("go_up") || match("move_up")){
			p->left = talloc(Prim_com);//左にPrim_comノードを作って
			parse(p->left);//Prim_com部分の構文解析の実行（左の子に丸投げ）

		} else {//Rep_comでもPrim_comでもなかったら
			err("Com_list: Rep_com node or Prim_com node expected."); //エラーを表示
		}
		...
		if (match("{") || match("walk") || match("move") || match("go_up") || match("move_up")) { //戻って来たら，さらにCom_listがあるか調べる
			...
			parse(p->right);//Com_list部分の構文解析の実行(右の子に丸投げ)
		}
	}
		...
		if (match("{")) {
			...
			err("Rep_com:integer required after token '{'.");// { がありません
		}
		...
		if (isnumber(tp)) {//繰り返し回数のチェック
			...
			err("Rep_com:integer required after token \"{\".");//繰り返し回数がありません
		}

		//左にCom_listノードを作って丸投げ
		//戻って来たら，閉じかっこ } があるか調べる．なければエラー

		p->left = talloc(Com_list);//左にCom_listノードを作る
		parse(p->left);//Com_list部分の構文解析の実行（左の子に丸投げ）
		...
		if (match("}")) {//'}'があったら
			...
			err("Rep_com:\"}\" required at the end of the node.");
		}
		...
		if (match("walk")) {
			...
			if (isfloat(tp)) {//第1引数のチェック
				...
				err("Prim_com:floating point number required.");//第1引数が正しくない
			}

			//第2引数チェック
			if (isfloat(tp)) {//自然数を表す記号列かどうか調べる

				p->fnum2 = atof(tp);//atof関数により自然数文字列を整数値に変換しfnum2に格納
				nextToken();

			} else {
				err("Prim_com:2 floating point numbers required.");
			}
			...
			if (isfloat(tp)) {//自然数を表す記号列かどうか調べる
				...
				p->fnum1 = atof(tp);//atof関数により自然数文字列を整数値に変換しfnum1に格納
				nextToken();

			} else {
				err("Prim_com:floating point number required.");
			}
			...
			if (isfloat(tp)) {//自然数を表す記号列かどうか調べる
				...
				p->fnum1 = atof(tp);//atoi関数により自然数文字列を整数値に変換しfnum1に格納
				nextToken();
