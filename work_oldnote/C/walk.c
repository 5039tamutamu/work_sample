...
void parse(struct tnode *p) {
	...
	if (p->type == Prog) {
	...
	} else if (p->type == Com_list) {//Com_listの場合

	  if(match("{")){//トークンが"{"だったら左側にRep_comノード作成＆parse呼び出し

	    p->left = talloc(Rep_com);
	    parse(p->left);

	  } else if (match("walk") ||match("move") ||match("go_up") ||match("move_up")){//"walk","move","go_up","move_up"のいずれかだったら左側にPrim_comノード作成＆parse呼び出し．
	    ...
	    err("parse Com_list: Rep_com or Prim_com  is expected: ");

	  } if(match("{") ||match("walk") ||match("move") ||match("go_up") ||match("move_up")){//戻って来たら，さらにCom_listがあるか調べ，あれば右にCom_listノードを作って丸投げ
	  ...
	  if (match("{")) {
			...
			err("parse Rep_com: { is expected: ");// { がありません
			...
			err("parse Rep_com: no isnumber: ");//繰り返し回数がありません
	  }if(match("{") ||match("walk") ||match("move") ||match("go_up") ||match("move_up")){

	  p->left = talloc(Com_list);//左にCom_listノードを作って丸投げ
	  parse(p->left);

	  } else {

	    err("parse Rep_com: Rep_com or Prime_com is expected: ");

	  } if (match("}")){//戻って来たら，閉じかっこ } があるか調べる．なければエラー

	    nextToken();

	  } else {

	    err("parse Rep_com: } is expected: ");

	  }

	} else if (p->type == Prim_com) {//Prim_comの場合
		...
		if (match("walk")) {
			...
			if (isfloat(tp)) {//第1引数のチェック
				...
				err("parse Prim_com: fnum1 isn't correct: ");//第1引数が正しくない
			}
			...
			if (isfloat(tp)) {//第2引数チェック
				...
				err("parse Prim_com walk: fnum2 isn't correct: ");//第2引数が正しくない
			}
...
/* exe関数（エグゼキュータ） */
  ...
  if (p->type == Prog) {//Progの場合
  ...
  } else if (p->type == Rep_com) {//Rep_comの場合

    for(int takanori=0;takanori<=p->inum;takanori++){
	...
	print_pos();
	pos_r += p->fnum1;//fnum1,fnum2の値を使ってpos_r,pos_thを更新
	pos_th += p->fnum2;
	print_pos();//xyz座標の表示
	
    } else if (p->op == MOVE) {//"move"の場合
