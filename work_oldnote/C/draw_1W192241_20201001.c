...
int main(int argc, char ** argv) {
	...
	s = (char *)malloc((ftell(infp) + 1)*sizeof(char));
	...
	for (i = 0; i < 100; i++) {
		...
		printf("%f\n",pt.da);
		//struct Point pt = {0, 0, 0, 2.23, stp};//この行は不要
		...
		while (tp != NULL) {
			if (match("[")) {
				...
				pt.stp += stp_growth * pt.stp;
			} else if (match("<")) {
				pt.stp -= stp_growth * pt.stp;
			} else if (match(")")) {
				pt.da += da_growth * pt.da;
			} else if (match("(")) {
				pt.da -= da_growth * pt.da;
			} else if (match("F")) {
				fprintf(outfp, "%f\t %f\n", pt.x, pt.y);
				pt.x += pt.stp * cos((pt.a / 360) * 2 * PI);
				pt.y += pt.stp * sin((pt.a / 360) * 2 * PI);
				//fprintf(outfp, "%f\t %f\n\n", pt.x, pt.y);
			} else {
				//上記以外のトークンは無視（何もしない）
			}
