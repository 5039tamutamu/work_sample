...
class GraphPlot: #クラス名の書き換え不可
    ...
    BR = np.zeros((3, 100)) #基線除去に用いる直前データの格納配列
    ...
    def Start(self):
        ...
        self.data1 = np.zeros(self.tmp) #プロットデータの格納配列(1ch)
        ...
        self.data2 = np.zeros(self.tmp) #プロットデータの格納配列(2ch)
        ...
        self.data3 = np.zeros(self.tmp) #プロットデータの格納配列(3ch)
    ...
    def Serial_Receive(self):
        ...
        for i in range(datasize):
            ...
            if head == b'\x80': #ヘッダ受信
                ...
                #データ整形
                A = ((d[0]<<7)+d[1]) - np.mean(self.BR[0])
                B = ((d[2]<<7)+d[3]) - np.mean(self.BR[1])
                C = ((d[4]<<7)+d[5]) - np.mean(self.BR[2])

                BR = np.delete(BR, 0, axis=1)
                BR = np.append(BR, [[A], [B], [C]], axis=1)
