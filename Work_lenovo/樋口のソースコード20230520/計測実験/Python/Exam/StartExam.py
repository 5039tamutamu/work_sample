...
class StartExam: #クラス名の書き換え不可
    ...
    def time_monitor(self):
        ...
        self.time_print = self.time_car - self.time_pre
        print(str(self.time_print) + " : " + sys._getframe().f_back.f_code.co_name)
    ...
    def Start(self): #実験開始用のボタンを提示する画面
        ...
        width=20, height=10, command=self.Calibration)
        self.Button1.pack(side=tk.LEFT)

        #「テストデータを取得」ボタンの生成
        self.Button2 = tk.Button(self.frame1, text="ジェスチャ計測(65秒)", 
        width=20, height=10, command=self.Test)
    ...
    def CalProccess_Rest(self, i):
        ...
        if self.list[i] <= 6:
            self.root2.after(self.delta_Rest, lambda:self.CalProccess_Gesture(i))
        else:
            self.root2.after(self.delta_Rest, self.Reset)
    ...
    def CalProccess_Gesture(self, i):
        ...
        self.root2.after(self.delta_Gesture, lambda:self.CalProccess_Rest(i+1))
        self.root2.mainloop()
    ...
    def TestProccess_Rest(self, i):
        ...
        self.Rest(self.sq[i])
        if self.sq[i] <= 6:
            self.root2.after(self.delta_Gesture, lambda:self.TestProccess_Gesture(i))
        else:
            self.root2.after(self.delta_Gesture, self.Reset)
    ...
    def TestProccess_Gesture(self, i):
        ...
        self.root2.after(self.delta_Gesture, lambda:self.TestProccess_Rest(i+1))
        self.root2.mainloop()
